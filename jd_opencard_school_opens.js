...
async function drawContent() {
    return new Promise(resolve => {
        let body = `activityId=46fcab8e90e243dcb1aa76a5c7c764d2&pin=${encodeURIComponent($.myPingData.secretPin)}`
        $.post(taskPostUrl('/dingzhi/taskact/openCardcommon/drawContent', body, `https://lzdz1-isv.isvjcloud.com/dingzhi/dz/openCard/activity/805745?activityId=f7e4d75f22c84cdeba92bf594b02b910&shareUuid=${$.shareUuid ? $.shareUuid : '63f49716ec334e44966bb26e79e34103'}`), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${$.name} API请求失败，请检查网路重试`)
                } else {
