...
入口
https://lzdz1-isv.isvjcloud.com/dingzhi/dz/openCard/activity/832865?activityId=078b967203634b208aaf65085d91a970&shareUuid=25b54d46093a4eac8e68d0f166923458

============Quantumultx===============
[task_local]
#8.11-8.15 星动七夕 纵向丝滑
36 0,8 4-12 8 * http://nm66.top/starqx.js, tag=8.11-8.15 星动七夕 纵向丝滑, enabled=true

================Loon==============
[Script]
cron "36 0,8 4-12 8 *" script-path=starqx.js,tag=8.11-8.15 星动七夕 纵向丝滑

===============Surge=================
8.11-8.15 星动七夕 纵向丝滑 = type=cron,cronexp="36 0,8 4-12 8 *",wake-system=1,timeout=3600,script-path=starqx.js

============小火箭=========
8.11-8.15 星动七夕 纵向丝滑 = type=cron,script-path=starqx.js, cronexpr="36 0,8 4-12 8 *", timeout=3600, enable=true
*/
const $ = new Env('8.11-8.15 星动七夕 纵向丝滑');
...
!(async () => {
  ...
  $.shareUuid = '25b54d46093a4eac8e68d0f166923458'
  console.log(`入口:\nhttps://lzdz1-isv.isvjcloud.com/dingzhi/dz/openCard/activity/832865?activityId=078b967203634b208aaf65085d91a970&shareUuid=${$.shareUuid}`)
...
function getDrawRecordHasCoupon() {
  return new Promise(resolve => {
    let body = `activityId=078b967203634b208aaf65085d91a970&actorUuid=${$.actorUuid}&pin=${encodeURIComponent($.myPingData.secretPin)}&num=0&sortSuatus=1`
...
function addSku() {
  return new Promise(resolve => {
    let body = `activityId=078b967203634b208aaf65085d91a970&pin=${encodeURIComponent($.myPingData.secretPin)}&actorUuid=${$.actorUuid}&taskType=2&taskValue=100022672084`
...
function followShop() {
  return new Promise(resolve => {
    let body = `activityId=078b967203634b208aaf65085d91a970&pin=${encodeURIComponent($.myPingData.secretPin)}&actorUuid=${$.actorUuid}&taskType=23&taskValue=1000009621&shareUuid=${$.shareUuid}`
...
function shopactivityId(functionId) {
  return {
    ...
    headers: {
      ...
      'Referer': `https://shopmember.m.jd.com/shopcard/?venderId=${functionId}&shopId=${functionId}&venderType=5&channel=401&returnUrl=https://lzdz1-isv.isvjcloud.com/dingzhi/dz/openCard/activity/832865?activityId=078b967203634b208aaf65085d91a970&shareUuid=${$.shareUuid}`,
      'Cookie': cookie
    }
  }
}
function startDraw(type) {
  return new Promise(resolve => {
    let body = `activityId=078b967203634b208aaf65085d91a970&actorUuid=${$.actorUuid}&pin=${encodeURIComponent($.myPingData.secretPin)}&type=${type}`
...
function checkOpenCard() {
  return new Promise(resolve => {
    let body = `activityId=078b967203634b208aaf65085d91a970&pin=${encodeURIComponent($.myPingData.secretPin)}&actorUuid=${$.actorUuid}&shareUuid=${$.shareUuid}`
...
function getMyPing() {
  ...
  return new Promise(resolve => {
    $.post({
      ...
      body: `userId=1000009621&token=${$.isvObfuscatorToken}&fromType=APP`,
      headers: {
        'User-Agent': $.UA,
        'Content-Type':'application/x-www-form-urlencoded',
        'Host':'lzdz1-isv.isvjcloud.com',
...
function getHtml() {
  ...
  return new Promise(resolve => {
    $.get({
      url: `https://lzdz1-isv.isvjcloud.com/dingzhi/dz/openCard/activity/832865?activityId=078b967203634b208aaf65085d91a970&shareUuid=${$.shareUuid}`,
      headers: {
        'User-Agent': $.UA,
        'Host':'lzdz1-isv.isvjcloud.com',
...
function adLog() {
  return new Promise(resolve => {
    let pageurl = `https://lzdz1-isv.isvjcloud.com/dingzhi/dz/openCard/activity/832865?activityId=078b967203634b208aaf65085d91a970&shareUuid=${$.shareUuid}`
    $.post({
      url: `https://lzdz1-isv.isvjcloud.com/common/accessLogWithAD`,
      body: `venderId=1000009621&code=99&pin=${encodeURIComponent($.myPingData.secretPin)}&activityId=078b967203634b208aaf65085d91a970&pageUrl=${encodeURIComponent(pageurl)}&subType=APP&adSource=null`,
      headers: {
        'User-Agent': $.UA,
        'Host':'lzdz1-isv.isvjcloud.com',
...
function drawContent() {
  return new Promise(resolve => {
    $.post({
      ...
      body: `activityId=078b967203634b208aaf65085d91a970&pin=${encodeURIComponent($.myPingData.secretPin)}`,
      headers: {
        'User-Agent': $.UA,
        'Host':'lzdz1-isv.isvjcloud.com',
        'Content-Type': 'application/x-www-form-urlencoded; Charset=UTF-8',
        'Referer':`https://lzdz1-isv.isvjcloud.com/dingzhi/dz/openCard/activity/832865?activityId=078b967203634b208aaf65085d91a970&shareUuid=${$.shareUuid}`,
        'Cookie': `LZ_TOKEN_KEY=${$.LZ_TOKEN_KEY}; LZ_TOKEN_VALUE=${$.LZ_TOKEN_VALUE}; AUTH_C_USER=${$.myPingData.secretPin}; ${$.lz_jdpin_token}`,
      }
    }, async (err, resp, data) => {
      try {
...
function getActorUuid() {
  return new Promise(resolve => {
    $.post({
      ...
      body: `activityId=078b967203634b208aaf65085d91a970&pin=${encodeURIComponent($.myPingData.secretPin)}&pinImg=${encodeURIComponent($.attrTouXiang)}&nick=${encodeURIComponent($.myPingData.nickname)}&cjyxPin=&cjhyPin=&shareUuid=${$.shareUuid}`,
      headers: {
        'User-Agent': $.UA,
        'Host':'lzdz1-isv.isvjcloud.com',
        'Content-Type': 'application/x-www-form-urlencoded; Charset=UTF-8',
