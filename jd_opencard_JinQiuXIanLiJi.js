...
入口复制到jd：
24.0复制整段话 https:/JKySMbfp3SG5bY 参与活动，瓜分百万京豆还有丰厚奖品等你拿#51sL62IawlC72e%しǎι【京東】ＡΡΡの
入口浏览器：
https://lzdz1-isv.isvjd.com/dingzhi/jdfresh/supercategory/activity/6197196?activityId=dz2109100010214101

更新地址：https://github.com/Tsukasa007/my_script
============Quantumultx===============
[task_local]
#9.13-9.27 京秋献礼季
5 0,5,18 * * * jd_opencard_JinQiuXIanLiJi.js, tag=9.13-9.27 京秋献礼季, img-url=https://raw.githubusercontent.com/tsukasa007/icon/master/jd_opencard_JinQiuXIanLiJi.png, enabled=true
*/
const $ = new Env('9.13-9.27 京秋献礼季');
...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        ...
        if (cookie) {
            ...
            let checkOpenCardData = await initOpenCard();
            if (!checkOpenCardData || !checkOpenCardData || !checkOpenCardData.data.openInfo) {
                continue
            }
            checkOpenCardData = checkOpenCardData.data.openInfo.filter(row =>{ return row.openStatus === false})
            if (checkOpenCardData.length > 0) {
                for (let cardListElement of checkOpenCardData) {
                    $.log('入会: ' + cardListElement.venderId)
                    await join(cardListElement.venderId)
                }
            } else {
                $.log("是否全部入会：true");
            }
...
function followShop() {
    return new Promise(resolve => {
        let body = `activityId=dz2109100010214101&pin=${encodeURIComponent($.myPingData.secretPin)}&actorUuid=${$.actorUuid}&taskType=23&taskValue=1000009621&shareUuid=${$.shareUuid ? $.shareUuid : '"00c7443e849347b29e06c18dcfd09c31"'}`
        $.post(taskPostUrl('/dingzhi/jdfresh/supercategory/followShop', body, 'https://lzdz1-isv.isvjcloud.com/dingzhi/jdfresh/supercategory/followShop'), async (err, resp, data) => {
            try {
                if (err) {
...
function getShareRecord() {
    return new Promise(resolve => {
        let body = `activityId=dz2109100010214101&actorUuid=${$.actorUuid}&shareUuid=${$.shareUuid ? $.shareUuid : '"00c7443e849347b29e06c18dcfd09c31"'}&pin=${encodeURIComponent($.myPingData.secretPin)}`
        $.post(taskPostUrl('/dingzhi/taskact/common/getShareRecord', body, 'https://lzdz1-isv.isvjcloud.com/dingzhi/taskact/openCardcommon/getDrawRecordHasCoupon'), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${$.name} API请求失败，请检查网路重试`)
                } else {
...
function saveTask(type,value) {
    return new Promise(resolve => {
        let body = `activityId=dz2109100010214101&pin=${encodeURIComponent($.myPingData.secretPin)}&taskType=${type}&taskValue=${value}&actorUuid=${$.actorUuid}&shareUuid=${$.shareUuid ? $.shareUuid : '"00c7443e849347b29e06c18dcfd09c31"'}`
        $.post(taskPostUrl('/dingzhi/jdfresh/supercategory/saveTask', body, 'https://lzdz1-isv.isvjcloud.com/dingzhi/jdfresh/supercategory/saveTask'), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${$.name} API请求失败，请检查网路重试`)
                } else {
...
async function startDraw(type) {
    return new Promise(resolve => {
        let body = `activityId=dz2109100010214101&actorUuid=${$.actorUuid}&shareUuid=${$.shareUuid ? $.shareUuid : '"00c7443e849347b29e06c18dcfd09c31"'}&pin=${encodeURIComponent($.myPingData.secretPin)}&type=${type}`
        $.post(taskPostUrl('/dingzhi/jdfresh/supercategory/draw', body, `https://lzdz1-isv.isvjcloud.com/dingzhi/jdfresh/supercategory/activity/805745?activityId=dz2109100010214101&shareUuid=${$.shareUuid ? $.shareUuid : '"00c7443e849347b29e06c18dcfd09c31"'}`), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${$.name} API请求失败，请检查网路重试`)
                } else {
                    data = JSON.parse(data);
                    if (data.result === true) {
                        $.log(`抽到了: ${data.data.wdsrvo ? data.data.prize.name : '毛一根'}`);
                    } else {
...
function getHtml() {
    return new Promise(resolve => {
        $.get({
            url: `https://lzdz1-isv.isvjcloud.com/dingzhi/jdfresh/supercategory/activity/797702?activityId=dz2109100010214101&shareUuid=${$.shareUuid ? $.shareUuid : '"00c7443e849347b29e06c18dcfd09c31"'}&adsource=null&shareuserid4minipg=P0CZ6sYjxiDL7YQZAjODCU7oeVP9kq2pYSH90mYt4m3fwcJlClpxrfmVYaGKuquQkdK3rLBQpEQH9V4tdrrh0w==&shopid=1000009621&lng=113.387958&lat=22.511013&sid=09fdc8e908526b5538a4ad4a265f40dw&un_area=19_1657_52093_0`,
            headers: {
...
function adLog() {
    return new Promise(resolve => {
        $.post({
            ...
            body: `venderId=1000009621&code=99&pin=${encodeURIComponent($.myPingData.secretPin)}&activityId=dz2109100010214101&pageUrl=https://lzdz1-isv.isvjcloud.com/dingzhi/jdfresh/supercategory/activity/797702?activityId=dz2109100010214101&shareUuid=${$.shareUuid ? $.shareUuid : '"00c7443e849347b29e06c18dcfd09c31"'}&adsource=null&shareuserid4minipg=P0CZ6sYjxiDL7YQZAjODCU7oeVP9kq2pYSH90mYt4m3fwcJlClpxrfmVYaGKuquQkdK3rLBQpEQH9V4tdrrh0w==&shopid=1000009621&lng=113.387958&lat=22.511013&sid=09fdc8e908526b5538a4ad4a265f40dw&un_area=19_1657_52093_0`,
            headers: {
                'User-Agent': `Mozilla/5.0 (Linux; U; Android 8.0.0; zh-cn; Mi Note 2 Build/OPR1.170623.032) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/61.0.3163.128 Mobile Safari/537.36 XiaoMi/MiuiBrowser/10.1.1`,
                'Host': 'lzdz1-isv.isvjcloud.com',
...
function getActorUuid() {
    return new Promise(resolve => {
        $.post({
            url: `https://lzdz1-isv.isvjcloud.com/dingzhi/jdfresh/supercategory/activityContent`,
            body: `activityId=dz2109100010214101&pin=${encodeURIComponent($.myPingData.secretPin)}&pinImg=https%3A%2F%2Fimg10.360buyimg.com%2Fimgzone%2Fjfs%2Ft1%2F7020%2F27%2F13511%2F6142%2F5c5138d8E4df2e764%2F5a1216a3a5043c5d.png&nick=${encodeURIComponent($.myPingData.nickname)}&cjyxPin=&cjhyPin=&shareUuid=${$.shareUuid ? $.shareUuid : '"00c7443e849347b29e06c18dcfd09c31"'}`,
            headers: {
                'User-Agent': `Mozilla/5.0 (Linux; U; Android 8.0.0; zh-cn; Mi Note 2 Build/OPR1.170623.032) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/61.0.3163.128 Mobile Safari/537.36 XiaoMi/MiuiBrowser/10.1.1`,
                'Host': 'lzdz1-isv.isvjcloud.com',
                'Content-Type': 'application/x-www-form-urlencoded; Charset=UTF-8',
                'Referer': 'https://lzdz1-isv.isvjcloud.com/dingzhi/jdfresh/supercategory/activityContent',
                'Cookie': `LZ_TOKEN_KEY=${$.LZ_TOKEN_KEY}; LZ_TOKEN_VALUE=${$.LZ_TOKEN_VALUE}; AUTH_C_USER=${$.myPingData.secretPin}; ${$.lz_jdpin_token}`,
            }
        }, async (err, resp, data) => {
            try {
...
function checkOpenCard() {
    return new Promise(resolve => {
        let body = `activityId=dz2109100010214101&actorUuid=${$.actorUuid}&shareUuid=${$.shareUuid}&pin=${encodeURIComponent($.myPingData.secretPin)}`
        $.post(taskPostUrl('/dingzhi/jdfresh/supercategory/checkOpenCard', body, 'https://lzdz1-isv.isvjcloud.com/dingzhi/jdfresh/supercategory/checkOpenCard'), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${$.name} API请求失败，请检查网路重试`)
                } else {
...
function initOpenCard() {
    return new Promise(resolve => {
        let body = `activityId=dz2109100010214101&actorUuid=${$.actorUuid}&shareUuid=${$.shareUuid}&pin=${encodeURIComponent($.myPingData.secretPin)}`
        $.post(taskPostUrl('/dingzhi/jdfresh/supercategory/initOpenCard', body, 'https://lzdz1-isv.isvjcloud.com/dingzhi/jdfresh/supercategory/initOpenCard'), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${$.name} API请求失败，请检查网路重试`)
                } else {
...
function taskPostUrl(url, body, referer) {
    return {
        ...
        headers: {
            ...
            "Referer": referer ? referer : `https://lzdz1-isv.isvjcloud.com/lzclient/dz/2021jan/eliminateGame/0713eliminate/?activityId=735c30216dc640638ceb6e63ff6d8b17&shareUuid=${$.shareUuid ? $.shareUuid : '"00c7443e849347b29e06c18dcfd09c31"'}&adsource=&shareuserid4minipg=u%2FcWHIy7%2Fx3Ij%20HjfbnnePkaL5GGqMTUc8u%2Fotw2E%20a7Ak3lgFoFQlZmf45w8Jzw&shopid=0&lng=114.062541&lat=29.541254&sid=57b59835c68ed8959d124d644f61c58w&un_area=4_48201_54794_0`,
            "User-Agent": UA,
